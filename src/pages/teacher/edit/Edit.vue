<template>
	<a-form-model :model="form" :label-col="{ span: 4 }" :wrapper-col="{ span: 14 }" ref="dailyForm" :rules="rules">
					...
					<a-collapse-panel key="6" header="上课时学生的言行举止或衣着不得体">
						<div :style="{ borderBottom: '1px solid #E9E9E9' }">
							<a-checkbox 
								...
								:checked="unImageCheckAll || form.unImageStudentList.length === unImageStudentOptions.length" 
			...
			<a-collapse-panel key="3" header="第三部分：条件保障">
				<a-collapse :default-active-key="[7, 8]">
					<a-collapse-panel key="7" header="投影仪不能正常使用">
						<div :style="{ borderBottom: '1px solid #E9E9E9' }">
							<a-checkbox :indeterminate="projectorNotAll" :checked="projectorCheckAll || form.projectorDamage.length == projectorStudentOptions.length" @change="(e) => onCheckAllChange(e, projectorParam)">全选</a-checkbox>
						</div>
						<br />
					...
					<a-collapse-panel key="8" header="电脑不能正常使用">
						<div :style="{ borderBottom: '1px solid #E9E9E9' }">
							<a-checkbox :indeterminate="computerNotAll" :checked="computerCheckAll || form.computerDamage.length === computerStudentOptions.length" @change="(e) => onCheckAllChange(e, computerParam)">全选</a-checkbox>
						</div>
						<br />
					...
					<a-collapse-panel key="9" header="其他问题">
						<div :style="{ borderBottom: '1px solid #E9E9E9' }">
							<a-checkbox :indeterminate="otherWarnNotAll" :checked="otherWarnCheckAll || form.otherWarnDamage.length === otherWarnStudentOptions.length" @change="(e) => onCheckAllChange(e, otherWarnParam)">全选</a-checkbox>
						</div>
						<br />
		...
		<a-form-model-item :wrapper-col="{ span: 100}">
			<a-button style="width: 100%;margin-top: 24px" size="large" type="primary" @click="submitForm" v-if="!isEdit">提交日报</a-button>
			<a-button style="width: 100%;margin-top: 24px" size="large" type="primary" @click="submitForm" v-if="isEdit">提交修改</a-button>
...
<script>
import {searchStudentList, getTeacherConfig, getClassStudent, editDaily, getDailyInfo} from '@/services/daily';
import moment from 'moment';
import debounce from 'lodash/debounce';
import { initCheckStudent, parseStudentInfo } from "@/utils/tools";
...
export default {
	...
	created() {
		...
		this.initSection();
		this.getTeacherConfigClassStudent().then(() => {
			// 这里必须使用promise来解决编辑模式下的异步问题
			if(this.$route.params.id){
				this.isEdit = true;
				this.dailyInfoId = Number(this.$route.params.id);
				this.initPageInfo();
			}
		}).catch((err) => {
			throw Error(err);
	...
	methods: {
		...
		// 修改模式下获取页面信息
		initPageInfo() {
			getDailyInfo(this.dailyInfoId).then((res) => {
				let resObj = res.data.data;

				this.form = {
					teacherName: this.user.nick_name,
					teacherId: this.user.id,
					classRoomName: resObj.class_room,//教室
					courseNature: resObj.course_nature, // 课程性质
					postDateTemp: moment(Number(resObj.post_date)), //上课时间(只作缓存使用，提交时会删除)
					postDate: Number(resObj.post_date),//上课时间
					submit_time: Number(resObj.submit_time), // 提交时间
					update_time: Number(resObj.update_time), // 更新时间
					section: resObj.section,//节次
					
					trueAttendNum: resObj.true_attend_num,//实到人数
					attendNum: resObj.attend_num,//应到人数
					useMedia: resObj.use_media, //使用媒体设备
					unImageStudentList: resObj.un_image_student ? resObj.un_image_student.split(",") : [], //衣着问题
					projectorDamage: resObj.projector_damage ? resObj.projector_damage.split(",") : [], //投影仪问题
					computerDamage: resObj.computer_damage ? resObj.computer_damage.split(",") : [], //电脑问题
					otherWarnDamage: resObj.other_warn_damage ? resObj.other_warn_damage.split(",") : [], //其他问题
					otherThings: resObj.other_things,//其他反馈
					// 教师配置项开始
					// 选中提交值
					course: resObj.course,//教师配置项-课程
					checkedClass: resObj.class.split(","),//本次课出席的班级
					checkedHeadTeacher: resObj.head_teacher.split(","),//本次课出席班级的班主任
					department: resObj.department,
					// 教师配置项结束
					
					checkedTruancyStudentList: resObj.truancy_student ? resObj.truancy_student.split(",") : [],//所选旷课学生列表
					checkedLateStudentList: resObj.late_student ? resObj.late_student.split(",") : [],//所选迟到学生列表
					checkedLeaveEarlyStudentList: resObj.leave_early_student ? resObj.leave_early_student.split(",") : [],//所选早退学生列表
					checkedLeaveStudentList: resObj.leave_student ? resObj.leave_student.split(",") : [],//所选请假学生列表
					
					truancyStudentNum: resObj.truancy_student_num,//旷课学生人数
					lateStudentNum: resObj.late_student_num,//迟到学生人数
					leaveEarlyStudentNum: resObj.leave_early_student_num,//早退学生人数
					leaveStudentNum: resObj.leave_student_num,//请假学生人数
				};

				let checkedTruancyStudentList = resObj.truancy_student ? resObj.truancy_student.split(",") : [];//所选旷课学生列表
				let checkedLateStudentList = resObj.late_student ? resObj.late_student.split(",") : [];//所选迟到学生列表
				let checkedLeaveEarlyStudentList = resObj.leave_early_student ? resObj.leave_early_student.split(",") : [];//所选早退学生列表
				let checkedLeaveStudentList = resObj.leave_student ? resObj.leave_student.split(",") : [];//所选请假学生列表

				// 初始化节次信息
				this.sectionValue = resObj.section.split("-").map(item => {
					return Number(item);
				});
				let childrenData = [];
				for (let i = this.sectionValue[0]; i <= this.sectionData.length ; i++) {
					childrenData.push({
						label: i,
						value: i
					})
				}
				this.sectionData[this.sectionValue[0]-1]['children'] = childrenData;
				// 配置选择的班级
				this.form.checkedClass.forEach(item => {
					this.selectCheckedClass(item);
				})
				
				// 配置选中学生项开始
				// 先分离数据，通过选择得到的学生数据
				this.form.checkedClass.forEach(className => {
					if (checkedTruancyStudentList.length > 0) {
						parseStudentInfo(checkedTruancyStudentList, className);
					}
					if(checkedLateStudentList.length > 0){
						parseStudentInfo(checkedLateStudentList, className);
					}
					if (checkedLeaveEarlyStudentList.length > 0) {
						parseStudentInfo(checkedLeaveEarlyStudentList, className);
					}
					if(checkedLeaveStudentList.length > 0){
						parseStudentInfo(checkedLeaveStudentList, className);
					}
				})
				// 再置默认选中项
				if (checkedTruancyStudentList.length > 0) {
					initCheckStudent(checkedTruancyStudentList, this.truancyStudentParams.selectData, this.studentList);
				}
				if(checkedLateStudentList.length > 0){
					initCheckStudent(checkedLateStudentList, this.lateStudentParams.selectData, this.studentList);
				}
				if (checkedLeaveEarlyStudentList.length > 0) {
					initCheckStudent(checkedLeaveEarlyStudentList, this.leaveEarlyStudentParams.selectData, this.studentList);
				}
				if(checkedLeaveStudentList.length > 0){
					initCheckStudent(checkedLeaveStudentList, this.leaveStudentParams.selectData, this.studentList);
				}
				// 配置选中学生项结束

				this.form.id = resObj.id;
			}).catch((err) => {
				throw Error(err);
			});
		},
		...
		submitForm() {
			this.$refs.dailyForm.validate(valid => {
				...
				this.form.update_time = (new Date()).getTime();
				editDaily(this.form).then((res) => {
					let resObj = res.data;
					if (resObj.code == 1) {
						try {
